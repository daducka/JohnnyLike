...
public static class OptimizerRunner
{
    ...
    /// comfort/safety/rest scaling, prep/fun suppression under distress,
    /// fun strengthening, prep pressure cap, and hunger suppression refinement.
    /// </summary>
    ...
    [
        ...
        new("StarvatingSatietyThreshold",
            ...
            Setter: (p, v) => WithMood(p, v,
                (m, val) => new MoodTuning
                {
                    ...
                    HungerSuppressionStartSatiety     = m.HungerSuppressionStartSatiety,
                    HungerSuppressionFullSatiety      = m.HungerSuppressionFullSatiety,
                    ComfortRestSuppressionMin         = m.ComfortRestSuppressionMin,
                    HungerSuppressionExponent         = m.HungerSuppressionExponent,
                })),
        ...
        // ── Fun strengthening ─────────────────────────────────────────────
        new("FunBaseScale",
            Min: 0.40, Max: 1.00, Step: 0.05,
            Getter: p => p.Mood.FunBaseScale,
            Setter: (p, v) => WithMood(p, v,
                (m, val) => new MoodTuning
                {
                    StarvatingSatietyThreshold        = m.StarvatingSatietyThreshold,
        ...
        // ── Prep pressure cap ─────────────────────────────────────────────
        new("PrepTimePressureCap",
            Min: 0.05, Max: 0.50, Step: 0.05,
            Getter: p => p.Need.PrepTimePressureCap,
            Setter: (p, v) => WithNeed(p, v,
                (n, val) => new NeedTuning
                {
                    FatiguePressureRestScale            = n.FatiguePressureRestScale,
                    ...
                    FoodConsumptionShareLow            = n.FoodConsumptionShareLow,
                    FoodShareNeutral                   = n.FoodShareNeutral,
                    PrepTimePressureCap                = val,
                    PrepTimePressureRatePerDay         = n.PrepTimePressureRatePerDay,
                })),

        // ── Hunger suppression refinement ─────────────────────────────────
        new("ComfortRestSuppressionMin",
            Min: 0.10, Max: 0.70, Step: 0.05,
            Getter: p => p.Mood.ComfortRestSuppressionMin,
            Setter: (p, v) => WithMood(p, v,
                (m, val) => new MoodTuning
                {
                    StarvatingSatietyThreshold        = m.StarvatingSatietyThreshold,
                    ...
                    HungerSuppressionStartSatiety     = m.HungerSuppressionStartSatiety,
                    HungerSuppressionFullSatiety      = m.HungerSuppressionFullSatiety,
                    ComfortRestSuppressionMin         = val,
                    HungerSuppressionExponent         = m.HungerSuppressionExponent,
                })),

        new("HungerSuppressionStartSatiety",
            Min: 15.0, Max: 40.0, Step: 2.5,
            Getter: p => p.Mood.HungerSuppressionStartSatiety,
            Setter: (p, v) => WithMood(p, v,
                (m, val) => new MoodTuning
                {
                    StarvatingSatietyThreshold        = m.StarvatingSatietyThreshold,
                    ...
                    InjuryMasterySuppressionFloor     = m.InjuryMasterySuppressionFloor,
                    InjuryPreparationSuppressionFloor = m.InjuryPreparationSuppressionFloor,
                    HungerSuppressionStartSatiety     = val,
                    HungerSuppressionFullSatiety      = m.HungerSuppressionFullSatiety,
                    ComfortRestSuppressionMin         = m.ComfortRestSuppressionMin,
                    HungerSuppressionExponent         = m.HungerSuppressionExponent,
                })),

        new("HungerSuppressionFullSatiety",
            Min: 5.0, Max: 20.0, Step: 2.5,
            Getter: p => p.Mood.HungerSuppressionFullSatiety,
            Setter: (p, v) => WithMood(p, v,
                (m, val) => new MoodTuning
                {
                    StarvatingSatietyThreshold        = m.StarvatingSatietyThreshold,
                    ...
                    InjuryPreparationSuppressionFloor = m.InjuryPreparationSuppressionFloor,
                    HungerSuppressionStartSatiety     = m.HungerSuppressionStartSatiety,
                    HungerSuppressionFullSatiety      = val,
                    ComfortRestSuppressionMin         = m.ComfortRestSuppressionMin,
                    HungerSuppressionExponent         = m.HungerSuppressionExponent,
                })),

        new("HungerSuppressionExponent",
            Min: 0.5, Max: 4.0, Step: 0.25,
            Getter: p => p.Mood.HungerSuppressionExponent,
            Setter: (p, v) => WithMood(p, v,
                (m, val) => new MoodTuning
                {
                    StarvatingSatietyThreshold        = m.StarvatingSatietyThreshold,
