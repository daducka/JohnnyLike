...
public class IslandDomainPack : IDomainPack
{
        ...
        IResourceAvailability resourceAvailability)
    {
        ...
        // Check if this action was generated by a ToolItem (has tool_item_id in ResultData)
        if (outcome.ResultData?.TryGetValue("tool_item_id", out var toolItemIdObj) == true)
        {
            var toolItemId = toolItemIdObj as string;
            if (toolItemId != null)
            {
                // Find the specific ToolItem that generated this action and apply its effects
                var toolItem = islandWorld.WorldItems.OfType<ToolItem>()
                    .FirstOrDefault(item => item.Id == toolItemId);
                
                if (toolItem != null)
                {
                    toolItem.ApplyEffects(effectCtx);
